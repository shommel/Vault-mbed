...
class Length(VarType):
    ...
    def setValue(self, value):
        ...
        #FIXME: removing the bytes(data) from line below because some values have longer length than 
        #       than 255 (would be outside of bytes operator)
...
class Message(object):
    ...
    def serialize(self):
        ...
        for i in range(1,1+len(self._fieldsLUT)):
            ...
            except KeyError: #catching some gaps in the protbuf field IDs. (For example, Features skips row 11)
                continue
        return data
